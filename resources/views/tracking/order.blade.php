                ...
                <div class="relative z-10 w-full overflow-x-auto timeline-scroll pb-4">
                    <div class="min-w-[600px] flex justify-between items-start relative pt-2 px-4 md:px-10">
                        ...
                        <div class="relative z-10 flex flex-col items-center w-28 group transition-all duration-500"
                             :class="(status === 'preparing' || status === 'ready' || status === 'served' || status === 'completed') ? 'opacity-100' : 'opacity-50'">
                            <div class="w-12 h-12 rounded-full flex items-center justify-center border-[2px] bg-[#040810] transition-all duration-500"
                                 :class="(status === 'preparing' || status === 'ready' || status === 'served' || status === 'completed') ? 'border-primary shadow-[0_0_15px_rgba(0,119,187,0.5)]' : 'border-white/20'">
                                <div class="w-3 h-3 rounded-full transition-all duration-500"
                                     :class="(status === 'preparing' || status === 'ready' || status === 'served' || status === 'completed') ? 'bg-primary' : 'bg-transparent'"></div>
                            </div>
                            <h3 class="mt-4 text-[12px] md:text-sm font-medium tracking-widest uppercase text-center transition-colors"
                                :class="(status === 'preparing' || status === 'ready' || status === 'served' || status === 'completed') ? 'text-white' : 'text-gray-500'">Đang nấu</h3>
                        </div>
                        ...
                        <div class="relative z-10 flex flex-col items-center w-28 group transition-all duration-500"
                            ...
                            <h3 class="mt-4 text-[12px] md:text-sm font-medium tracking-widest uppercase text-center transition-colors"
                                :class="(status === 'served' || status === 'completed') ? 'text-white' : 'text-gray-500'">Đã phục vụ</h3>
                        </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('orderTracker', () => ({
                ...
                getProgressWidth() {
                    ...
                    if (this.status === 'served' || this.status === 'completed') return '66%';
                    if (this.status === 'ready') return '66%';
                    if (this.status === 'preparing') return '33%';
                    return '0%';
                },
