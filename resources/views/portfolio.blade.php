            ...
            <div id="portfolio-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($foods as $food)
                    <div class="portfolio-item relative aspect-[3/4] overflow-hidden group cursor-pointer" data-category="{{ $food->category->slug ?? '' }}">
                        ...
                        <!-- Heart Icon -->
                        <button class="absolute top-4 right-4 z-20 w-10 h-10 bg-black/40 backdrop-blur-sm rounded-full flex items-center justify-center text-red-500 hover:scale-110 transition-transform btn-wishlist" data-id="{{ $food->id }}">
                            @php
